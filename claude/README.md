...
成果物: `target/kit_teamfight-1.0-SNAPSHOT-shaded.jar` (shade 済み、約66KB)。これを Paper サーバの `plugins/` に入れる。
(`original-kit_teamfight-1.0-SNAPSHOT.jar` は shade 前のものなので使わない。)
...
├── game
│   ├── GameManager          ハブ。チーム/プレイヤー/装備/購入/チケット/勝敗/馬/スコアボード
│   ├── GameConfig           config.yml の値を保持
│   ├── GameTask             1秒ループ (ポイント/旗キャプチャ/ドレイン/サイドバー)
│   ├── FlagParticleTask     0.5秒ループ (占領範囲パーティクル)
│   └── Team                 チーム1つ (チケット・スポーン・色)
├── player
│   └── PlayerData           プレイヤーの所属/ポイント/保有kit
├── kit
│   ├── Kit                  kit インターフェース (枠組み・騎兵/馬の既定値)
│   ├── KitRegistry          kit の登録・取得
│   ├── KitShop              チェストGUI + クリック処理
│   └── kits/                実装 kit 7種 (archer/skirmisher/soldier/arbalest/
│                            lancer/robinhood/sniper) + DefaultKit (初期装備)
├── flag
│   ├── Flag                 旗1つ (所有チーム・キャプチャ進捗)
│   ├── FlagManager          旗の生成/削除/flags.yml 永続化
│   └── FlagVisualizer       旗ブロック/ホログラム/占領リングの描画
├── command
│   ├── GameCommand          /ktf
│   ├── TeamCommand          /team
│   ├── KitCommand           /kit
│   └── FlagCommand          /flag
└── listener
    └── PlayerListener       参加/退出/死亡(キル報酬)/リスポーン/騎乗制限/FF無効化/旗ブロック破壊
```
